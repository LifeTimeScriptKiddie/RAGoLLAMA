...
Manages document metadata, chunks, and comparison results in SQLite database.
"""
...
class SQLiteManager:
    """SQLite database manager for document processing pipeline."""
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("""
                    CREATE TABLE IF NOT EXISTS chunks (
                        ...
                        UNIQUE(document_id, chunk_index, embedding_method)
                    )
                """)
                ...
                conn.execute("CREATE INDEX IF NOT EXISTS idx_chunks_method ON chunks(embedding_method)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_comparisons_document ON comparisons(document_id)")
    ...
    def add_chunk(self, document_id: int, chunk_index: int, chunk_text: str,
                  embedding_method: str, vector_id: Optional[int] = None, 
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("""
                    ...
                    (document_id, chunk_index, chunk_text, chunk_hash, chunk_length,
                     embedding_method, vector_id, metadata)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    document_id,
                    chunk_index,
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("""
                    ...
                    (document_id, docling_chunks, microsoft_chunks, 
                     similarity_scores, comparison_results, best_method)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    document_id,
                    json.dumps(comparison_data.get("docling_chunks", [])),
                    json.dumps(comparison_data.get("microsoft_chunks", [])),
                    json.dumps(comparison_data.get("similarity_scores", [])),
                    json.dumps(comparison_data.get("comparison_results", {})),
                    comparison_data.get("best_method", "")
                ))
    ...
    def get_chunks(self, document_id: int, embedding_method: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get chunks for a document, optionally filtered by embedding method."""
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
    ...
    def get_comparison(self, document_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if row:
                    comp = dict(row)
                    comp["docling_chunks"] = json.loads(comp["docling_chunks"]) if comp["docling_chunks"] else []
                    comp["microsoft_chunks"] = json.loads(comp["microsoft_chunks"]) if comp["microsoft_chunks"] else []
                    comp["similarity_scores"] = json.loads(comp["similarity_scores"]) if comp["similarity_scores"] else []
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("SELECT COUNT(DISTINCT embedding_method) FROM chunks")
